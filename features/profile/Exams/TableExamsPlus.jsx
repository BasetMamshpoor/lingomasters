...
const TableExams = ({data, isLoading, id}) => {
    return (
        <>
            <Table
                fullWidth
            ...
            >
                <TableHeader>
                    ...
                    <TableColumn>دانلود</TableColumn>
                </TableHeader>
                <TableBody
                    isLoading={isLoading}
                ...
                >
                    {data?.map(item => (
                        <TableRow key={item.id}>
                            ...
                            <TableCell>{item.type ? "داخل ایران" : "خارج ایران"}</TableCell>
                            ...
                            <TableCell>
                                <Link href={`/students/${id}/exam-score?exam_id=${item.id}`}
                                      className="centerOfParent gap-3 text-primary-600">
                                    <Download className="fill-primary-600"/>
                                    دانلود کارنامه
                                </Link>
                            </TableCell>
                        </TableRow>
                            ...
                            <AccordionItem
                                ...
                                title={
                                    <div className="grid grid-cols-3 gap-2 text-xs">
                                        <span className="text-natural_gray-950">نام آزمون</span>
                                        <span
                                            className="col-span-2 text-black font-semibold flex items-center gap-3"><Verify
                                            className="fill-primary-600"/>{item.exam_name}</span>
                                    </div>
                                }>
                                ...
                                <p className="text-natural_gray-950">نوع آزمون</p>
                                <p className="col-span-2 font-semibold">{item.type ? "داخل ایران" : "خارج ایران"}</p>
                                ...
                                <Link href={`/students/${id}/exam-score?exam_id=${item.id}`}
                                      className="flex items-center col-span-2"><Dots className="rotate-90"/></Link>
                                <p className="text-natural_gray-950">دانلود کارنامه</p>
                                <Link href={`/students/${id}/exam-score?exam_id=${item.id}`}
                                      className="flex items-center gap-3 text-primary-600 col-span-2">
                                    <Download className="fill-primary-600"/>
                                    دانلود کارنامه
                                </Link>
                            </AccordionItem>
