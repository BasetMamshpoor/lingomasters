...
export default function ClassCancellation({id, disclosure, is_near}) {
    ...
    const handleSubmit = async (e) => {
        ...
        const {
            ...
            successMessage,
            success
        } = await sendPostRequest('POST', `/privet-class/update_cancellation/${id}`, formData, true)
        if (success)
            addToast({
                title: 'انجام شد',
                            ...
                            <ModalBody>
                                <div className="flex flex-col gap-6">
                                    {!!is_near && <Alert
                                        ...
                                        description='به دلیل اینکه لغو کلاس زیر 24 ساعت انجام می‌شود باید دلیل موجه داشته باشید و گواهی آن را آپلود کنید ، در غیر این صورت مبلغ جریمه از کیف شما کسر خواهد شد.'
                                    />}
                                    <div className="relative w-full">
                                        <Textarea minRows={5} radius="sm"
                                                  label='دلیل لغو کلاس'
                                                  labelPlacement='outside'
                                                  value={formData.reason}
                                                  onChange={handleInputChange}
                                                  maxRows={10} variant="bordered" name="reason"
                                                  className="self-stretch" placeholder="توضیحات">توضیحات</Textarea>
                                        <div
                                            ...
                                            <span
                                                ...
                                                <Close className='fill-primary-600'/> : 'انتخاب غیبت موجه'}</span>
                                        </div>
                                    </div>
                                    <div
                                        onDrop={handleDrop}
