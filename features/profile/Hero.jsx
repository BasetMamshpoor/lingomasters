...
import ProfileImage from "@/features/profile/ImageProfile";

const Hero = ({level, children, isSuggestion, identification_code}) => {
    return (
        <>
            <div className="flex flex-col gap-6 self-center items-center w-full">
                <div className="flex flex-col gap-3 items-center">
                    <div className="flex flex-col gap-2">
                        <ProfileImage justImage={isSuggestion}/>
                        <p className='flex items-center gap-2'>
                            <span className='text-xs text-natural_gray-950'>کد زبان‌آموزی:</span>
                            <span className='text-sm'>123456</span>
                        </p>
                    </div>
                    {!isSuggestion && <div className='flex items-center gap-1'>
                ...
                {!level ?
                    <div className='flex flex-col gap-2 p-6 border border-natural_gray-200 rounded-xl bg-white w-full'>
                ...
                <div
                    ...
                    <div className="flex items-center gap-3">
                        <p className='sm:text-sm text-xs text-natural_gray-950'>{identification_code}</p>
                        <button
                            onClick={() => {
                                window.navigator.clipboard.writeText(`https://lingomasters.ir/auth/register?code=${identification_code}`);
                                addToast({
                                    title: 'کپی شد',
