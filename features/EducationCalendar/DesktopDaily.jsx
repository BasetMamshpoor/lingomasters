...
const DesktopDaily = ({date, selected, update}) => {
    const [weekDays, setWeekDays] = useState([]);
    const [classes, , , , , isLoading] = useGetRequest(true, `/calendar/daily?date=${date}&type=${selected}`);
                ...
                <div className="grid grid-cols-7">
                    {weekDays.map((day, index) => (
                        <div
                            ...
                            className={`flex flex-col gap-3 cursor-pointer items-center p-3 border-y border-l  ${index === 0 ? 'border-r rounded-r-lg' : index === 6 ? 'rounded-l-lg' : ''} ${isCurrentDay(day) ? 'bg-primary-600 text-white border-transparent' : 'bg-white text-natural_gray-900 border-natural_gray-200'}`}
                        >
                            {toJalaliDate(day)}
                        </div>
                    ))}
            ...
            }
            {
                (!isLoading && !dataLoading)
                    ...
                    <div className="flex gap-6">
                        ...
                        <div
                            className="sm:w-[86.5%] w-[84%] border border-natural_gray-200 rounded-lg overflow-hidden">
                            {(() => {
                                                        ...
                                                        >
                                                            <div className="flex flex-col gap-4">
                                                                <div
                                                                    ...
                                                                    <span
                                                                        className="whitespace-nowrap text-natural_gray-800">
                                                                        {currentClass.type}
                                                                    </span>
                                                                ...
                                                                {span > 1 && (
                                                                    <div
                                                                        ...
                                                                        <span>{currentClass.sesion || 'سوم'}</span>
                                                                    </div>
                                                                )}
                                                            </div>
                                                            ...
                                                            {(currentClass.teachingType === 2 && !currentClass.past) ? (
                                                                ...
                                                                <Link href={'/'}
                                                                      className="centerOfParent border-1.5 rounded border-secondary-500 font-semibold text-secondary-500 px-4 py-2 w-32">جزئیات</Link>
                                                            )}
                                                        </div>
                                                    </div>
                                                );
