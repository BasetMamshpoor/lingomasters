...
const Item = ({levelDetermination, attention, title, text, ul, button, image, titleImage}) => {
    return (
        <>
            <div
                className="flex flex-col gap-4 md:gap-8 lg:gap-10 px-3 py-4 md:px-6 md:py-8 lg:px-12 lg:py-14 border border-primary-200 rounded-3xl bg-[rgba(255,_255,_255,_0.50)] backdrop-filter backdrop-blur grow">
                ...
                <div className="flex flex-col gap-8">
                    <p className="text-base md:text-2xl lg:text-5xl font-bold text-primary-600">{title}</p>
                ...
                <div className="flex flex-col gap-4 w-full">
                    ...
                    {image && <Image unoptimized src={image} alt="" height={100} width={100} className="w-full"/>}
                </div>
                <div className="flex w-full justify-end">
                    <Link href="/" className="flex w-fit text-white px-4 py-2 bg-primary-600 rounded-md text-xs md:text-sm lg:text-base">{button}</Link>
                </div>
            </div>
        </>
    );
