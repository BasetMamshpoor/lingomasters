...
import GroupCard from "@/features/profile/classes/GroupCard";
import ClassItem from "@/features/group-class/List/ClassItem";

const Index = () => {
    const [currentPage, setCurrentPage] = useState(1)
    const [selected, setSelected] = useState("reserved");
    const [data, , , pagination, , isLoading] = useGetRequest(true, `/student/group?type=${selected}`, currentPage);
    const [data2, , , , , isLoading2] = useGetRequest(true, `/student/group/suggestion`);
    return (
        <div className="flex flex-col gap-8">
                ...
                {isLoading ?
                    ...
                    : data.length ?
                        <div className="grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 lg:gap-6 sm:gap-5 gap-4">
                            {data.map(e => <GroupCard selected={selected} key={e.id} data={e}/>)}
            ...
            <div className="flex flex-col gap-6">
                <div className="flex items-center gap-6 justify-between">
                    <p className="text-primary-950 font-bold">کلاس‌های گروهی بیشتر</p>
                ...
                {isLoading2 ?
                    ...
                    : data2.length ?
                        <div
                            ...
                            {data2.map(e => <ClassItem key={e.id} data={e}/>)}
                        </div> : <Alert description="کلاسی وجود ندارد" color="warning"/>}
            </div>
        </div>
    );
