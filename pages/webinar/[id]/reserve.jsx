...
const Reserve = () => {
    ...
    const [data, setData, setReload, paginations, setPaginations, loading] = useGetRequest(false, query.id && `/webinar-reserve/info/${query.id}`)
    ...
    const handlePay = async () => {
        ...
        const {
            ...
            status,
            errorMessage
        } = await sendPostRequest("POST", `/webinar-reserve/pay${selected === '2' ? "/wallet" : ""}/${id}`, {code: coupon?.code || null}, false, true)
        if (success) {
            if (selected === '1')
                push(Data.url)
            else {
                    ...
                    {data ? <> <Progress active={1} steps={stepsList} page={data.title} title="وبینارها"
                        ...
                        <div className="grid lg:grid-cols-4 grid-cols-1 lg:gap-6 gap-y-6 my-10">
                            ...
                            <div
                                ...
                                <CheckCoupon id={data?.id} setCoupon={setCoupon} model="webinar" />
                            </div>
                        </div>
